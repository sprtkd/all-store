...
import { UiUser } from "../user/models/user-model";
...
export default function SideDrawer(props: SideBarProps) {
  ...
  function UserListLoggedIn(props: UiUser) {
    return (
      <List>
  ...
  function AccountLinksListLoggedIn() {
    return (
      <List>
        <ListItem button key={2} component={Link} to="/logout">
          <ListItemIcon>
            <CancelIcon />
          </ListItemIcon>
          <ListItemText primary={"Logout"} />
        </ListItem>
        <ListItem button key={3} component={Link} to="/device">
          <ListItemIcon>
            <DevicesIcon />
          </ListItemIcon>
          <ListItemText primary={"This Device"} />
        </ListItem>
        <ListItem button key={4} component={Link} to="/my-apps">
          <ListItemIcon>
            <AssignmentIcon />
          </ListItemIcon>
          <ListItemText primary={"My Apps"} />
        </ListItem>
        <ListItem button key={5} component={Link} to="/wishlist">
          <ListItemIcon>
            <LibraryAddCheckIcon />
          </ListItemIcon>
  ...
  function TopAppName() {
    return (
      <Button
        ...
        component={Link}
        to={"/"}
        startIcon={<LocalMallIcon style={{ fontSize: 35 }} />}
      >
        AllStore
      </Button>
    );
  ...
  function SideBarList() {
    return (
      <div
        ...
        role="presentation"
        onClick={props.callbackSidebarToggle(false)}
        onKeyDown={props.callbackSidebarToggle(false)}
      >
        <TopAppName />
        <Divider />
        <UserContext.Consumer>
          {({ user, setValue }) =>
            user.isLoggedIn && (
              <div>
                <UserListLoggedIn {...user} />
                <Divider />
                <AccountLinksListLoggedIn />
              </div>
            )
          }
        </UserContext.Consumer>
  ...
  return (
    <div>
      <Drawer
        ...
        onClose={props.callbackSidebarToggle(false)}
      >
        <SideBarList />
      </Drawer>
    </div>
